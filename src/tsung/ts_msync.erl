...
%% @spec decode_buffer(Buffer::binary(),Session::record(msync)) ->  NewBuffer::binary()
%% @doc We need to decode buffer (remove chunks, decompress ...) for
%%      matching or dyn_variables
%% @end
decode_buffer(Buffer,#msync_session{}) ->
    Buffer. % nothing to do for msync


%% @spec userid({Session::record(msync_session), Dynvars::dynvars()}) ->  UID::string()
%% @doc  return the current userid  @end
userid({#msync_session{username=UID},_DynVars})-> UID.
...
%% Returns: binary
%%----------------------------------------------------------------------
get_message(Req=#msync{domain={domain,Domain}}, State=#state_rcv{session=S}) when S#msync_session.domain == undefined  ->
    NewS = S#msync_session{domain=Domain, user_server=default},
    get_message(Req#msync{domain=Domain, user_server=default},State#state_rcv{session=NewS});

get_message(Req=#msync{domain={vhost,FileId}}, State=#state_rcv{session=S}) when S#msync_session.domain == undefined  ->
    {Domain,UserServer} = choose_domain(FileId),
    NewS = S#msync_session{domain=Domain, user_server=UserServer},
    get_message(Req#msync{domain=Domain, user_server=UserServer},State#state_rcv{session=NewS});

get_message(Req=#msync{id=user_defined, appkey=Appkey, username=User, passwd=Passwd}, State=#state_rcv{session=S}) when S#msync_session.id == undefined  ->
    NewS = S#msync_session{id=user_defined,appkey=Appkey,username=User,passwd=Passwd},
...
get_message(Req=#msync{appkey=Appkey, prefix=Prefix, passwd=Passwd}, State=#state_rcv{session=S}) when S#msync_session.id == undefined  ->
   Id = case ts_user_server:get_idle(S#msync_session.user_server) of
    ...
    {NewAppkey,NewUser,NewPasswd} = {Appkey, username(Prefix,Id), password(Passwd,Id)},
    ...
    NewS = S#msync_session{id=Id,appkey=NewAppkey,username=NewUser,passwd=NewPasswd},

    get_message(Req#msync{appkey=NewAppkey,username=NewUser,passwd=NewPasswd},State#state_rcv{session=NewS});

get_message(Req=#msync{},#state_rcv{session=S}) ->
    {ts_msync_common:get_message(Req),S}.
...
starttls_bidi(_RcvdXml, #state_rcv{socket= Socket, send_timestamp=SendTime}=State)->
    ...
    Opt = lists:filter(fun({_,V}) -> V /= undefined end, 
                      [{certfile,Req#msync.certfile},
                       {keyfile,Req#msync.keyfile},
                       {password,Req#msync.keypass},
                       {cacertfile,Req#msync.cacertfile}]),
...
parse_config(Element, Conf) ->
    ts_config_msync:parse_config(Element, Conf).
...
%% The rest of the code expect to found a "domain" field in the #msync request
%% with the domain of the msync server (as string). We use the step of dynvars substitution
...
add_dynparams(Subst, {DynVars, S}, Param=#msync{}, Host) when S#msync_session.id == undefined ->
    add_dynparams2(Subst,DynVars, Param, Host);

add_dynparams(Subst, {DynVars, S}, Param=#msync{}, Host) ->
    add_dynparams2(Subst,DynVars, Param#msync{id=S#msync_session.id,
                                               appkey=S#msync_session.appkey,
                                               username=S#msync_session.username,
                                               passwd=S#msync_session.passwd,
                                               domain=S#msync_session.domain,
                                               user_server=S#msync_session.user_server},Host).


add_dynparams2(false,_, Param, _Host) ->
    Param;
add_dynparams2(true, DynVars, Param, _Host) ->
    ?DebugF("Subst in msync msg (~p) with dyn vars ~p~n",[Param,DynVars]),
...
%%----------------------------------------------------------------------
subst(Req=#msync{id=user_defined, appkey=Appkey, username=Name,passwd=Pwd, data=Data, resource=Resource}, Dynvars) ->
    NewAppkey = ts_search:subst(Appkey,Dynvars),
    NewUser = ts_search:subst(Name,Dynvars),
    NewPwd  = ts_search:subst(Pwd,Dynvars),
    NewData = ts_search:subst(Data,Dynvars),
    subst2(Req#msync{appkey=NewAppkey,username=NewUser,passwd=NewPwd,data=NewData,resource=ts_search:subst(Resource,Dynvars)}, Dynvars);

subst(Req=#msync{data=Data,resource=Resource}, Dynvars) ->
    subst2(Req#msync{data=ts_search:subst(Data,Dynvars),resource=ts_search:subst(Resource,Dynvars)},Dynvars).

subst2(Req=#msync{type = Type}, Dynvars) when Type == 'starttls' ->
    Req#msync{cacertfile = ts_search:subst(Req#msync.cacertfile, Dynvars),
               keyfile = ts_search:subst(Req#msync.keyfile, Dynvars),
               keypass = ts_search:subst(Req#msync.keypass, Dynvars),
               certfile = ts_search:subst(Req#msync.certfile, Dynvars)};
subst2(Req=#msync{type = Type}, Dynvars) when Type == 'muc:chat' ; Type == 'muc:join'; Type == 'muc:nick' ; Type == 'muc:exit' ->
    Req#msync{nick = ts_search:subst(Req#msync.nick, Dynvars),
               room = ts_search:subst(Req#msync.room, Dynvars)};
subst2(Req=#msync{type = Type}, Dynvars) when Type == 'muc:info' ->
    Req#msync{room = ts_search:subst(Req#msync.room, Dynvars)};
subst2(Req=#msync{type = Type}, Dynvars) when Type == 'pubsub:create' ; Type == 'pubsub:subscribe'; Type == 'pubsub:publish'; Type == 'pubsub:delete' ->
    Req#msync{node = ts_search:subst(Req#msync.node, Dynvars)};
subst2(Req=#msync{type = Type}, Dynvars) when Type == 'pubsub:unsubscribe' ->
    NewNode=ts_search:subst(Req#msync.node,Dynvars),
    NewSubId=ts_search:subst(Req#msync.subid,Dynvars),
    Req#msync{node=NewNode,subid=NewSubId};
subst2(Req, _Dynvars) ->
    Req.

%%----------------------------------------------------------------------
%% Func: updatejab/2
%%  takes dyn vars and adds them to msync record
...
updatejab([{nonce, Val}|Rest], Param)->
   updatejab(Rest, Param#msync{nonce = Val});
updatejab([{sid, Val}|Rest], Param)->
   updatejab(Rest, Param#msync{sid = Val});
